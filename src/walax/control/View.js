...
export default class View extends Control {
  ...
  get login () {
    return {
      view: () =>
        m(
          '.login#login',
          m('form.login#loginForm', { method: 'post' }, [
            m(
              'input[type=text][autocomplete=username].login.loginName#username'
            ),
            m(
              'input[type=password][autocomplete=current-password].login.loginPass#password'
            ),
  ...
  display () {
    ...
    this._pages.forEach(v => {
      if (!defPage || v.default) defPage = v.url
      pages[v.url] = v.page
      navs.push(m('a.nav', { href: '#!' + v.url }, v.nav))
    })

    this.d(
      'building display routes',
      { main },
      { defPage },
      { pages },
      { navs }
  ...
  addNavLogo (label, img = undefined) {
    this.nav.innerHTML =
      '<div class="logo">' + label + '</div>' + this.nav.innerHTML
  }
}
